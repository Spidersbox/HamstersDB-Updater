...
string icon="icon";// name of your icon in the res file
string DirName="HamstersDB";
string downlocation="https://republichams.net/Downloads";
QString downloadLocation;
string AppName="HamstersDB";
//string apppath="C:\\"+DirName+"\\";

std::string FileName;
std::string strAppDir = GetDefaultAppDir().string();
//  datadir is if you want to put things in (windows appdata) or (linux ~/.dirname)
//  so in windows: c:/users/username/appdata/roaming/App
//  in linux /home/username/.app
//  otherwise datadir = appdir
//
//std::string strDataDir = GetDefaultDataDir().string();
std::string strDataDir = strAppDir;
...
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":images/"+QString::fromStdString(icon)));
    setWindowIcon(QIcon(":images/"+QString::fromStdString(icon)));
#else
...
//  temp="checking "+downloadLocation.toStdString();
//  ui.TextEdit->appendPlainText(temp.c_str());
...
//  spiderbyte does not use lowercase
